...
start_date_str = "09-07-2020"
end_date_str = "27-07-2020"
...
adp_url = (
    "https://adfs.navya.tech/adfs/ls/idpinitiatedsignon.aspx?logintoRP=ADP-HRServices"
)
...
while True:
    ...
    if start_adp_date < start_date:
        week_array = list(
            filter(lambda x: (x >= 2 * (start_date - start_adp_date).days), week_array)
        )
        print(
            "Half-days before",
    ...
    if end_adp_date > end_date:
        week_array = list(
            filter(lambda x: (x <= 13 - 2 * (end_adp_date - end_date).days), week_array)
        )
        print(
            "Half-days after",
